			  ...
			  struct avalon_dma_desc *desc)
{
	...
	if (desc->direction == DMA_MEM_TO_DEV) {
		...
		ep_dst_hi = fw_params.rd_ep_dst_hi;
		ep_dst_lo = fw_params.rd_ep_dst_lo;
		...
		ep_dst_hi = fw_params.wr_ep_dst_hi;
		ep_dst_lo = fw_params.wr_ep_dst_lo;
			   ...
			   unsigned int irq)
{
	...
	ret = dma_set_mask_and_coherent(dev, DMA_BIT_MASK(fw_params.dma_mask_width));
	if (ret)
		return ret;
				    ...
				    struct dma_slave_config *config)
{
	 ...
	 * me: no, these are controlled by IPs and not configurable
	 */
			 ...
			 unsigned long flags, void *context)
{
	 ...
	 * me: fixed? (below); should -1 be changed to INVALID_DMA_ADDR or something?
	 */
				       ...
				       unsigned int irq)
{
	 ...
	 * me: yes, I do not want to mix styles and stick to no-API only.
	 */
	...
	dma_dev->src_addr_widths = BIT(DMA_SLAVE_BUSWIDTH_UNDEFINED);
	dma_dev->dst_addr_widths = BIT(DMA_SLAVE_BUSWIDTH_UNDEFINED);
	dma_dev->max_sg_burst = AVALON_DMA_DESC_NUM;
