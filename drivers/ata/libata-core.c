...
{
	int tag = percpu_ida_alloc(&ap->qc_internal, GFP_ATOMIC);
...
{
	int tag = percpu_ida_alloc(&ap->qc_tags, GFP_ATOMIC);
...
struct ata_port *ata_port_alloc(struct ata_host *host)
{
	...
	if (__percpu_ida_init(&ap->qc_tags,
			      ATA_MAX_QUEUE - 1, 4, 2, ATA_MAX_QUEUE / 2) < 0) {
		kfree(ap);
		return NULL;
	}

	if (__percpu_ida_init(&ap->qc_internal, 1, 1, 1, 0) < 0) {
