...
static int do_rtas_setup_msi_irqs(struct pci_dev *pdev, int nvec_in)
{
	struct pci_dn *pdn;
	int hwirq_base, virq_base, i, rc;
	struct msi_desc *entry;
	struct msi_msg msg;
	int nvec = roundup_pow_of_two(nvec_in);
...
static int do_rtas_setup_msix_irqs(struct pci_dev *pdev, int nvec_in)
{
	struct pci_dn *pdn;
	int hwirq, virq, i, rc, m, quota;
	struct msi_desc *entry;
	struct msi_msg msg;
	int nvec = nvec_in;
